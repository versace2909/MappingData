...
`POST /api/data-mapping` SHALL create a new `DataMapping` row with Status = `New`, persisting MappingName, SourceDataId, TargetDataId, CreatedDate, and CreatedBy.
...
- **THEN** the system SHALL return 201 with the created mapping's id and status = "New"
...
- **THEN** the system SHALL return 400 with a descriptive error message
